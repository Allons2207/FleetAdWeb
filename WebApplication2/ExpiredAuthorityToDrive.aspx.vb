...
Public Class ExpiredAuthorityToDrive
    ...
    Private Sub loadData()
        Dim sql As String = "SELECT        PersonnelNumber, FirstName, LastName, Position, DATEDIFF(month, GETDATE(), [AU-DRVE]) AS Months, DATEDIFF(day, GETDATE(), [AU-DRVE]) AS DATE, [AU-DRVE]
FROM            dbo.vwUpdateDrivers
WHERE        (DATEDIFF(month, GETDATE(), [AU-DRVE]) = 1) OR
                         (DATEDIFF(month, GETDATE(), [AU-DRVE]) = 0) OR
                         (DATEDIFF(month, GETDATE(), [AU-DRVE]) IS NULL)"
    ...
    Protected Sub gvCustomers_Load(sender As Object, e As EventArgs)
        For Each dr As GridViewRow In gvCustomers.Rows
            Try
                If dr.RowType = DataControlRowType.DataRow Then
                    Dim chkRow As LinkButton = TryCast(dr.Cells(7).FindControl("lnkstat"), LinkButton)
                    Dim aa As Int32 = Convert.ToInt32(dr.Cells(6).Text)
                    If aa > 10 Then
