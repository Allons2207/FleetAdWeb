...
Public Class ExamSubjectPapersMarkConsolidation
    ...
    Protected Sub cmdSearch_Click(sender As Object, e As EventArgs) Handles cmdSearch.Click
        Dim sql As String = "SELECT DISTINCT examId AS [Exam_Code], Paper, highestPossibleMark AS [Marked_Out_Of], 0 AS [Select], 0 AS [Assigned_Percentage_Contribution] FROM dbo.tbl_examinations " & _
                          " WHERE (subject = '" & cboSubject.Text & "') AND (stream = '" & cboStreams.Text & "') AND  ( (examDate >= '" & rdpDateFrom.SelectedDate & "') AND (examDate <= '" & rdpDateTo.SelectedDate & "')) "
        With gwTests
            Try
    ...
    Protected Sub cmdApply_Click(sender As Object, e As EventArgs) Handles cmdApply.Click
        ...
        Dim sql As String = "UPDATE [tbl_examinationMarks] SET [selected] = 0, percentWeight = 0 "
        Try
        ...
        For Each gridRow As GridDataItem In gwTests.Items
            If gridRow.Selected Then
                ...
                If chkProcessForClass.Checked = True Then
                    sql = "UPDATE [tbl_examinationMarks] SET [selected] = 1, percentWeight = " & Val(txtBox.Text) & " WHERE [examId] = '" & gridRow.Item("Exam_Code").Text & "' AND schClass = '" & cboClasses.Text & "'"
                Else
                    sql = "UPDATE [tbl_examinationMarks] SET [selected] = 1, percentWeight = " & Val(txtBox.Text) & " WHERE [examId] = '" & gridRow.Item("Exam_Code").Text & "'"
                End If
        ...
        If chkProcessForStream.Checked = True Then
            sql = "DELETE tbl_examinationMarksAggregatedBySubject WHERE  [intYear] = " & txtYear.Text & " AND [term] = '" & cboTerm.Text & "' AND  [subject] = '" & cboSubject.Text & "' AND [stream] = '" & cboStreams.Text & "'"
        ElseIf chkProcessForClass.Checked = True Then
            sql = "DELETE tbl_examinationMarksAggregatedBySubject WHERE  [intYear] = " & txtYear.Text & " AND [term] = '" & cboTerm.Text & "' AND  [subject] = '" & cboSubject.Text & "' AND [stream] = '" & cboStreams.Text & "' AND [schClass] = '" & cboClasses.Text & "'"
        End If
        ...
        sql = "INSERT [dbo].[tbl_examinationMarksAggregatedBySubject] ([studentId], [subject], [intYear], [term], [percentMarkObtained], [selected], [stream], [schClass]) " & _
                            " (SELECT [studentId], '" & cboSubject.Text & "' AS [subject], " & txtYear.Text & " AS [intYear], '" & cboTerm.Text & "' AS [term], SUM(CAST((percentWeight/100)*percentage AS integer)) AS " & _
                            " [markObtained], 1 , [stream], [schClass] FROM [tbl_examinationMarks] WHERE [Selected] = 1  GROUP BY StudentId, [stream], [schClass]) "
        ...
        Try
            ...
            If ds.Tables.Count > 0 Then
                For ctrRow = 0 To ds.Tables(0).Rows.Count
                    ...
                    qry = "UPDATE tbl_examinationMarksAggregatedBySubject SET grade = '" & grade & "' WHERE [selected] = 1  AND (percentMarkObtained >= " & fromMark & " AND percentMarkObtained <= " & toMark & ")"
            ...
            If ds.Tables.Count > 0 Then
                For ctrRow = 0 To ds.Tables(0).Rows.Count
                    ...
                    For ctrMak = intMark To 0 Step -1
                        qry = "UPDATE [tbl_examinationMarksAggregatedBySubject] SET classPosition = " & poz & " WHERE [selected] = 1 AND percentMarkObtained = " & ctrMak & " AND schClass ='" & schClass & "'"
                        obj.ExecuteNonQRY(qry)
    ...
    Private Sub generateClassPositionsForSpecifiedClass()
        ...
        For ctrMak = intMark To 0 Step -1
            Dim qry As String = "UPDATE tbl_examinationMarksAggregatedBySubject SET classPosition = " & poz & " WHERE [selected] = 1 AND percentMarkObtained = " & ctrMak & " AND schClass ='" & cboClasses.Text & "'"
    ...
    Private Function numberOfSameClassStudentsWithSameTestMark(ByVal mark As Integer, ByVal schClass As String) As Integer
        Dim qry As String = "SELECT COUNT(StudentId) AS numberOfStudents FROM [tbl_examinationMarksAggregatedBySubject] WHERE [selected] = 1 AND percentMarkObtained = " & mark & " AND schClass = '" & schClass & "'"
    ...
    Private Function generateStreamPositions() As Integer
        ...
        For ctrMak = intMark To 0 Step -1
            Dim qry As String = "UPDATE [tbl_examinationMarksAggregatedBySubject] SET streamPosition = " & poz & " WHERE [selected] = 1 AND percentMarkObtained = " & ctrMak
    ...
    Private Function numberOfStudentsWithSameTestMark(ByVal mark As Integer) As Integer
        Dim qry As String = "SELECT COUNT(StudentId) AS numberOfStudents FROM [tbl_examinationMarksAggregatedBySubject] WHERE [selected] = 1 AND percentMarkObtained = " & mark
