...
Public Class ZinaraLicensing
    ...
    Private Sub loadExistingLicenseDetails()
        Dim qry As String = "SELECT dbo.tbl_ZinaraLicensing.fleetId, dbo.tbl_ZinaraLicensing.licenseNum, dbo.tbl_ZinaraLicensing.dateIssued," &
                            " dbo.tbl_ZinaraLicensing.expiryDate, dbo.tbl_ZinaraLicensing.commentsNotes, dbo.tbl_vehicleMakes.vehicleMake, " &
                            " dbo.tbl_vehicleModels.vehicleModel, tbl_vehicleData.registrationNumber as regNumber FROM   dbo.tbl_ZinaraLicensing INNER JOIN dbo.tbl_vehicleModels INNER JOIN " &
                            ...
                            " dbo.tbl_ZinaraLicensing.fleetId = dbo.tbl_vehicleData.fleetId WHERE " &
                            " (dbo.tbl_ZinaraLicensing.fleetId = '" & (Request.QueryString("op")) & "')"
    ...
    Protected Sub cmdSave_Click(sender As Object, e As EventArgs) Handles cmdSave.Click
        ...
        If txtFleetIDSearch.Text = "" Then
            obj.MessageLabel(lblMsg, "Please specify the vehicle's fleet number, it cannot be empty.", "Red")
            Exit Sub
        ...
        Dim qry As String = "DELETE FROM tbl_ZinaraLicensing WHERE fleetId = '" & txtFleetIDSearch.Text & "'"

        obj.ConnectionString = con

        If obj.ExecuteNonQRY(qry) = 1 Then
            qry = "INSERT tbl_ZinaraLicensing (fleetId,licenseNum,dateIssued,expiryDate,commentsNotes) VALUES ('" &
                    txtFleetIDSearch.Text & "','" & txtLicenseNumber.Text & "','" & rdDateIssued.SelectedDate & "','" & rdExpiryDate.SelectedDate & "','" & txtCommentsNotes.Text & "')"

            If obj.ExecuteNonQRY(qry) = 1 Then
                obj.MessageLabel(lblMsg, "Entry successfully saved.", "Green")
            End If
        Else
    ...
    Private Sub loadVehicleData()
        ...
        Catch ex As Exception
            obj.MessageLabel(lblMsg, "No such vehicle found under vehicle data.", "Yellow")
        End Try
    ...
    Protected Sub cmdDelete_Click(sender As Object, e As EventArgs) Handles cmdDelete.Click
        If Not IsNothing(Request.QueryString("op")) Then
            Dim qry As String = "DELETE FROM tbl_ZinaraLicensing WHERE (dbo.tbl_ZinaraLicensing.fleetId = '" & (Request.QueryString("op")) & "')"
            obj.ConnectionString = con

            If obj.ExecuteNonQRY(qry) = 1 Then
                obj.MessageLabel(lblMsg, "Zinara License Record successfully deleted.", "Green")
            Else
                obj.MessageLabel(lblMsg, "Error while trying to delete Zinara License entry.", "Red")
            End If
        Else
            obj.MessageLabel(lblMsg, "Error. System could not find the Vehicle Fleet Number.", "Red")
        End If
    End Sub
