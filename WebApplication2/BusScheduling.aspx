<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="BusScheduling.aspx.vb" Inherits="WebApplication2.BusScheduling" %>
        ...
        <nav class="navbar navbar-main navbar-expand-lg position-sticky mt-4 top-1 px-0 mx-4 shadow-none border-radius-xl z-index-sticky" id="navbarBlur" data-scroll="true">
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb px-3">
                    ...
                    <h6 class="font-weight-bolder mb-0 px-3">Bus Scheduling </h6>
                </nav>
    ...
    $(document).ready(function () {
        ...
        function initE() {
            ...
            $.ajax({
                ...
                dataType: "json",
                url: "BusScheduling.aspx/GetCalendarData",
                success: function (dataDB) {
                    var re = JSON.parse(dataDB.d);
        ...
        function GenerateCalendar(events) {
            var calendar = new FullCalendar.Calendar(document.getElementById("calendar"), {
                eventDidMount: function (info) {
                    $(info.el).tooltip({
        title:  "Bus" + ": " +info.event.extendedProps.fleet,
        placement: 'top',
                        trigger: 'hover',
        container: 'body'
                    });
                ...
                eventClick: function (event) {
                     ...
                     $(event.el).tooltip('hide')
                    selectedEvent = event
                    $('#modal-title-notification').html(event.event.title);
                    $('#Bus').html(event.event.extendedProps["fleet"]);
                    $('#Driver').html(event.event.extendedProps["driver"]);
        ...
        function openAddEditForm() {
            if (selectedEvent != null) {
                $('#EventID').val(selectedEvent.event.id);
                $('#title').val(selectedEvent.event.title);
                $('#task').val(selectedEvent.event.extendedProps["description"]);
                $('#route').val(selectedEvent.event.extendedProps["route"]);
                $('#DropDownList2').val(selectedEvent.event.extendedProps["driver"]);
                $('#fleetid').val(selectedEvent.event.extendedProps["fleet"]);
                $('#color').val(selectedEvent.event.color);
            }
        ...
        $('#save').click(function () {
            ...
            var datasave = {
                ...
                start: $('#datetime').val().trim(),
                EventDescription: $('#task').val(),
                Driver: $('#driver').val(),
                FleetId: $('#fleetid').val(),
                Rute: $('#route').val(),
                color: $('#color').val(),
        ...
        function SaveEvent(datasave) {
            $.ajax({
                type: "POST",
                url: "BusScheduling.aspx/SaveUser",
                data: '{datasave: ' + JSON.stringify(datasave) + '}',
    ...
    window.location.reload();
        ...
        $('#delete').click(function () {
            if (selectedEvent != null && confirm('Are you sure?')) {
                var datasave = {
                    evID: selectedEvent.event.id,
                };
                $.ajax({
                    type: "POST",
                    url: "BusScheduling.aspx/DelUser",
                    data: '{datasave: ' + JSON.stringify(datasave) + '}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data) {
